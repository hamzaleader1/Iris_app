...
classifier=st.sidebar.selectbox("Choose Classifier",("Support Vector Machine","Logistic Regression","Random Forest Classifier"))

if st.sidebar.button("Predict"):
	if classifier=="Support Vector Machine":
		ans=prediction(svc_model,sepal_length,sepal_width,petal_length,petal_width)
		ans2=svc_model.score(X_train,y_train)
	elif classifier=="Logistic Regression":
		ans=prediction(log_reg,sepal_length,sepal_width,petal_length,petal_width)
		ans2=log_reg.score(X_train,y_train)
	else:
		ans=prediction(rf_clf,sepal_length,sepal_width,petal_length,petal_width)
		ans2=rf_clf.score(X_train,y_train)
	st.write(f'The predicted species is {ans}')
	st.write(f'The accuracy of the model is {ans2}')
